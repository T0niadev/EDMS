...
@section('content')
  ...
  <div class="p-4 h-full my-6 mx-16 sm:ml-64 lg-64">
    <div class="p-4 border-2 border-gray-200 border rounded-lg dark:border-gray-700 mt-14">
      ...
      <div class="flex h-10 mb-10 rounded">
          ...
          <button onclick="document.getElementById('folder1-modal').style.display='block'"
            class="btn h-9 min-w-[7rem] mt-2 mr-4 rounded dark:text-white font-medium border text-gray-900">
            Create Folder

          </button>
          <button onclick="document.getElementById('file1-modal').style.display='block'"
      ...
      <div class="grid grid-cols-7 gap-2 mb-4 ">
         @foreach ($folderas as $foldera)
          <div class="h-30 rounded">
        
            <div>
              <p class="text-2xl text-gray-400 dark:text-gray-500">
                <a href="{{ url($foldera->id, 'subfolders') }}">
                    <img
                    class="w-14"
            ...
            <div class="mt-1 flex items-center w-30">
              ...
              <a href="{{ url('folder/edit',$foldera->id) }}"><svg class="h-4 w-6 text-gray-800 dark:text-white"  viewBox="0 0 24 24"  xmlns="http://www.w3.org/2000/svg"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round">  <path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z" /></svg></button>
            
              <span class="flex-1 text-left whitespace-nowrap" sidebar-toggle-item>
                <a href="{{ url('tier/share',$foldera->id) }}">
          ...
          @endforeach
          @foreach ($fileas as $filea)
          <div class="h-30 rounded">
            <div>
              <p class="text-2xl text-gray-400 dark:text-gray-500">
                <a href="{{ asset($filea->document) }}">
            ...
            <div class="mt-1 flex items-center w-30">
              ...
              <a href="{{ url('folder/edit',$filea->id) }}"><svg class="h-4 w-6 text-gray-800 dark:text-white"  viewBox="0 0 24 24"  xmlns="http://www.w3.org/2000/svg"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round">  <path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z" /></svg></button>

              <span class="flex-1 text-left whitespace-nowrap" sidebar-toggle-item>
                <a href="{{ url('1sttier/edit',$filea->id) }}">
                  <svg class="h-4 w-6 text-gray-800 dark:text-white"  fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/>
                  </svg>
                </a>
    ...
    <div id="folder1-modal" style="top: 10%; left: 500px;" class="fixed top-40 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
      <div class="relative w-full max-w-2xl max-h-full">
          ...
          <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
              ...
              <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                  ...
                  <button   onclick="document.getElementById('folder1-modal').style.display='none'"  type="button" data-modal-hide="folder1-modal"  class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white">
              ...
              <div class="p-6 space-y-6">
                <form id="deptadd" method="POST" action="{{ url('/folder/store') }}" enctype="multipart/form-data">
                  @csrf
                  <label class="block">
                  ...
                  <input value="{{ $department->id }}" name="department_id"
    ...
    <div id="file1-modal" style="top: 10%; left: 500px;" class="fixed top-40 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
      <div class="relative w-full max-w-2xl max-h-full">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <!-- Modal header -->
            <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                ...
                <button   onclick="document.getElementById('file1-modal').style.display='none'"  type="button" data-modal-hide="file1-modal"  class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white">
                    <svg class="w-3 h-3" aria-hidden="true"  xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                    </svg>
                    <span class="sr-only">Close modal</span>
            ...
            <!-- Modal body -->
            <div class="p-6 space-y-6">
              <form id="fileadd" method="POST" action="{{ url('/folder/store') }}"enctype="multipart/form-data">
                @csrf
                <label class="block">
  ...
  <script>
    ...
    var modal = document.getElementById('folder1-modal');
    var modal2 = document.getElementById('file1-modal');
    var modal3 = document.getElementById('edit1-modal');
