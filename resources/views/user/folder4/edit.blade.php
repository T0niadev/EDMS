...
@section('content')
  ...
  <main class="main-content w-full px-[var(--margin-x)] pb-8">
      ...
      <div class="flex items-center space-x-1">
        ...
        <div class="flex my-1 h-10 mb-2 rounded border-bottom">
          <p class="text-2xl text-gray-900 dark:text-white">
              <a href="{{ url($folderd->folderc->folderb->foldera->department->id, 'folders') }}"  >{{$folderd->folderc->folderb->foldera->department->name}} >></a>
              <a href="{{ url($folderd->folderc->folderb->foldera->id, 'subfolders') }}" > {{$folderd->folderc->folderb->foldera->name}} >> </a>
              <a href="{{ url($folderd->folderc->folderb->id, 'subfolders/2ndtier') }}"> {{$folderd->folderc->folderb->name}} >> </a>
              <a href="{{ url($folderd->folderc->id, 'subfolders/3rdtier') }}"> {{$folderd->folderc->name}} >> </a>
              {{$folderd->name }}
          </p>
        </div>
      </div>
    ...
    <form  method="POST" action="{{ url('3rdtier/update',$folderd->id) }}" enctype="multipart/form-data">
                ...
                <div class="card space-y-5 p-4 sm:p-5">
                  <label class="block">
                    ...
                    <input 
                      ...
                      placeholder="" type="text"  name="name" value="{{$folderd->name}}"/>
                  </label>
