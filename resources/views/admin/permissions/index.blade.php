...
@section('content')
          ...
          <tbody>
            ...
            @foreach($permissions as $permission)
              <tr class="border-y border-transparent border-b-slate-200 dark:border-b-navy-500">
                ...
                <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                    <button onclick="document.getElementById('edit{{$permission->id}}').style.display='block'"
                ...
                <div id="edit{{$permission->id}}" style="top: 10%; left: 500px;" class="fixed top-40 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                    <div class="relative w-full max-w-2xl max-h-full">
                        ...
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            ...
                            <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                                ...
                                <button   onclick="document.getElementById('edit{{$permission->id}}').style.display='none'"  type="button" data-modal-hide="dept-modal"  class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white">
                            ...
                            <div class="p-6 space-y-6">
                                <form id="permadd" method="POST" action="{{ url('admin/permission/update',$permission->id) }}" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <label class="block">
                                    <span class="font-medium text-slate-600 dark:text-navy-100">Permission Name</span>

                                    <input 
                                    class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                    placeholder="" name="name" value="{{$permission->name}}" type="text" />
                                </label>
    ...
    <div id="dept-modal" style="top: 10%; left: 500px;" class="fixed top-40 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
      <div class="relative w-full max-w-2xl max-h-full">
          ...
          <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
              ...
              <!-- Modal body -->
              <div class="p-6 space-y-6">
                <form id="deptadd" method="POST" action="{{ url('admin/permission/store') }}" enctype="multipart/form-data">
                  @csrf
                  <label class="block">
                    <span class="font-medium text-slate-600 dark:text-navy-100">Permission Name</span>
  ...
  <script>
    ...
    <script>
    // Get the modal
    var modal = document.getElementById('edit{{$permission->id}}');
