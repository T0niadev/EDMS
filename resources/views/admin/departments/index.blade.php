...
@section('content')
      ...
      <div class="grid grid-cols-7 gap-2 mb-4">
          ...
          <div class="h-24 rounded">
            ...
            <div class="mt-1 flex items-center w-30">
              ...
              <button  onclick="document.getElementById('edit{{$department->id}}').style.display='block'"><svg class="h-4 w-6 text-gray-800 dark:text-white"  viewBox="0 0 24 24"  xmlns="http://www.w3.org/2000/svg"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round">  <path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z" /></svg></button>


            </div>
            <div  id="edit{{$department->id}}"  style="top: 10%; left: 500px;" class="fixed top-40 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
              <div class="relative w-full max-w-2xl max-h-full">
                  ...
                  <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                      ...
                      <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                          ...
                          <button  onclick="document.getElementById('edit{{$department->id}}').style.display='none'"  type="button" data-modal-hide="edit-modal"  class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white">
                      ...
                      <div class="p-6 space-y-6">
                        <form id="deptedit" method="POST" action="{{ url('admin/department/update', $department->id) }}" enctype="multipart/form-data">
                          @csrf
                          @method('PUT')
                          <label class="block">
                            <span class="font-medium text-slate-600 dark:text-navy-100">Department Name</span>

                            <input 
                              class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                              placeholder="" value="{{$department->name}}" name="name" type="text" />
                          </label>
    ...
    <div id="dept-modal" style="top: 10%; left: 500px;" class="fixed top-40 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
      <div class="relative w-full max-w-2xl max-h-full">
          ...
          <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
              ...
              <!-- Modal body -->
              <div class="p-6 space-y-6">
                <form id="deptadd" method="POST" action="{{ url('admin/department/store') }}" enctype="multipart/form-data">
                  @csrf
                  <label class="block">
                    <span class="font-medium text-slate-600 dark:text-navy-100">Department Name</span>
  ...
  <script>
    ...
    var model = document.getElementById('edit{{$department->id}}');
